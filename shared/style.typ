...
#let setup(doc, title: "", size: 8.8pt, lang: "en") = {
  ...
  set page(
    ...
    header: if draft {
      align(right, text(size: 6pt, fill: draft-mark)[PROTOTYPE · DRAFT])
    },
    header-ascent: 40%,
  )
...
// ---------------------------------------------------------------------
// Visuals. An ID either has an asset or it does not; one that does not
// keeps its hatched placeholder box, so an unfinished visual stays visible
// as unfinished instead of quietly disappearing.
//
// Typst cannot test whether a file exists, so tools/scan-visuals.py does
// the looking and writes visuals/manifest.json (regenerated by build.sh,
// and committed so a plain `typst compile` also works).
//
// Each visual is recorded as metadata either way, so build.sh can report
// which ones are still open.

#let _visuals = json("/visuals/manifest.json")
...
#let placeholder(id, brief, width: 100%, height: 30mm, compact: false) = {
  let asset = _visuals.at(id, default: none)
  [#metadata((id: id, brief: brief, height: repr(height), asset: asset)) <visual>]
  if asset != none {
    block(
      width: width,
      height: height,
      spacing: 0.6em,
      image(asset, width: 100%, height: 100%, fit: "contain"),
    )
  } else {
    block(
      width: width,
